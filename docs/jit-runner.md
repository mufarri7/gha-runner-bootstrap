...
It must contain only a regular file named `admission.json`, no directory or link entries. Evidence schema 2 is strict and includes repository, workflow path/name/id, run ID/attempt, admission job ID/name, PR number, exact base/head/merge/tree SHAs, label, generation timestamp, and a trusted `workload_boundary` attestation. That attestation must require a job container, enumerate at least one digest-pinned job image plus any digest-pinned service images, and prove that custom container options and volumes are empty. Missing, legacy, mutable-image, duplicate-image, host-option, or host-volume attestations fail closed. `ghrctl` limits compressed and expanded size, verifies GitHub's SHA-256 artifact digest, validates the artifact's run/repository/branch/SHA metadata, and requires list/get metadata stability. A stale attempt, duplicate artifact, unexpected field, unsafe archive, digest mismatch, or value mismatch also fails closed.

The attestation must be produced by the trusted admission workflow from its validated workflow graph, not by candidate code. Until that producer emits schema 2, admission intentionally remains blocked; this controller change does not modify the external workflow repository.
...
The runner and its dedicated Rootless Docker daemon execute in one transient systemd boundary with private network, mount, `/tmp`, `/var/tmp`, `/dev/shm`, and IPC namespaces. A separate deterministic root systemd service attaches `slirp4netns` to that namespace. Its exact startup bounding set is `CAP_SETPCAP`, `CAP_NET_BIND_SERVICE`, `CAP_NET_ADMIN`, `CAP_SYS_PTRACE`, and `CAP_SYS_ADMIN`: these are needed to drop the bounding set, preserve the low-port capability, configure the target network namespace, open cross-UID namespace descriptors, and enter/create the sandbox namespaces. The controller verifies the host capability name/number mapping and the exact startup mask before protected-path checks. After `slirp4netns` creates its sandbox, `/proc` must show only `CAP_NET_BIND_SERVICE` in inheritable, permitted, effective, and bounding sets, with no ambient capability; PID start ticks are checked on both sides of that read. Any missing startup capability or retained startup-only capability fails closed.

Launch also fails unless the installed build advertises both sandbox and libseccomp support; the helper runs with `--enable-sandbox`, `--enable-seccomp`, and `--disable-host-loopback`. `NoNewPrivileges`, strict system/home protection, private mounts and temporary storage, bounded address families, and inaccessible controller state, logs, application key, root home, and host Docker sockets limit its host view. Before processing traffic, the unit verifies those paths are unreadable and then replaces its environment with the fixed system-only PATH. The worker unit hides host homes, restricts writable paths to the worker boundary, and is killed as one cgroup.

Candidate steps may execute only in the attested job container. The listener's runner root, `.runner`, `.credentials*`, JIT configuration, process namespace, and Rootless Docker control socket are not mounted into that container. Because the stock runner requests `/var/run/docker.sock` for job containers, the worker mount namespace supplies a root-owned, read-only, non-socket sentinel at that path; it never binds the Rootless Docker socket. Custom container options and volumes are prohibited by the trusted attestation. Each slot therefore has distinct loopback services, temporary files, shared memory, IPC objects, home, daemon socket, and Docker data root. Identity teardown (`usermod`, `userdel`, `groupdel`, subordinate-map removal) and its final map/mount validation take the same host-mutation lock as allocation, so a replacement cannot observe a half-removed worker. Workers use exactly:
